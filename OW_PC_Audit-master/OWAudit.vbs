...
userFullName = objShell.ExpandEnvironmentStrings("%USERNAME%")
userFullDomain = objShell.ExpandEnvironmentStrings("%USERDOMAIN%")

'Script to change a filename using timestamps
strPath = userProfilePath &"\Desktop\" 'Place resultant file on the Users Desktop
...
if objFSO.FileExists(strPath & userFullName & "_" & strFileName & "_Audit.txt") then
WScript.Quit
end if

'Set the file location to collect the data
Set objFSO = CreateObject("Scripting.FileSystemObject")
Set objTextFile = objFSO.OpenTextFile(strPath & userFullName & "_" & strFileName & "_Audit.txt", ForAppending, True)
